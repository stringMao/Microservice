...
func RegisterAccount(c *gin.Context) {
	...
	re, err := redis.String(redisCon.Do("GET", str))
	if err != nil {
...
func ApplySMSVerificationCode(c *gin.Context) {
	...
	//检查是否已经发送过了，且还未过期
	keyexit, _ := redis.Bool(redisCon.Do("EXISTS", strlock))
	if keyexit == true {
	...
	//验证码保存到redis
	_, err := redisCon.Do("SETEX", str, 5*60, rndCode)
	if err != nil {
		log.Logger.Errorln("redis write [smscode] err:", err)
		return
	}
	redisCon.Do("SETEX", strlock, 60, "短信已发送")
