...
func (t *WechatUserInfo) Get(openid string) bool {
	has, err := dbmanager.Get_LoginSvr().Where("openid=?", openid).Get(t)
	if err != nil {
...
func (t *WechatUserInfo) GetRefreshToken(openid string) string {
	has, err := dbmanager.Get_LoginSvr().Where("openid='?'", openid).Cols("refreshtoken").Get(t)
	if err != nil {
		log.WithFields(log.Fields{
			"has":    has,
			"err":    err,
...
func (t *WechatUserInfo) InsertOrUpdate() bool {
	has, err := dbmanager.Get_LoginSvr().Where("openid = ?", t.Openid).Exist(&Token{})
	if err != nil {
		log.WithFields(log.Fields{
			"has":         has,
			"err":         err,
	...
	if has {
		//存在则更新
		affected, err := dbmanager.Get_LoginSvr().Where("openid = ?", t.Openid).Update(t)
		if err != nil {
			log.WithFields(log.Fields{
				"affected":    affected,
				"err":         err,
