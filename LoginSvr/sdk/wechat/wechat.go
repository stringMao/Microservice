...
func GetAccessTokenByCode(c *gin.Context) {
	...
	pub.InsertTokenToRedis(token, acc) //token放进redis

	//=========================================================
	redisCon := dbmanager.GetRedis()
	defer redisCon.Close()
	//将access_token有效期信息保存在redis
	redisCon.Do("SET", fmt.Sprintf("Wechat_actoken_%s", ret.Openid), ret.AccessToken, "EX", ret.ExpriesIn-5)
...
func LoginByAccessToken(c *gin.Context) {
	...
	if re, err := redis.String(redisCon.Do("GET", str)); err == nil {
